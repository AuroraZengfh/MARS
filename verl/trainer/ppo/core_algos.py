...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        for (sub_index, idx) in enumerate(id2score):
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            elif len(id2score[idx]) > 1:
                id2std[idx] = torch.std(torch.cat((torch.tensor([id2score[idx]]), mono_score[sub_index*num_mono:(sub_index+1)*num_mono].reshape(1,num_mono)), dim=-1))
                id2mean[idx] = torch.mean(torch.cat((torch.tensor([id2score[idx]]), mono_score[sub_index*num_mono:(sub_index+1)*num_mono].reshape(1,num_mono)), dim=-1))
            else:
